package com.mny.mojito.im.data.api.message;
...
public class MsgCreateModel {
    ...
    public MessageCard buildCard() {
        if (card == null) {
            ...
            card.setType(type);
            card.setSenderId(AccountManager.INSTANCE.getUserId());
    ...
    public static MsgCreateModel buildWithMessage(Message message) {
        ...
        model.id = message.getMessage().getServerId();
        model.content = message.getMessage().getContent();
        model.type = message.getMessage().getType();
        model.attach = message.getMessage().getAttach();

        if (message.getReceiver() != null) {
            // 如果接收者不为null，则是给人发送消息
            model.receiverId = message.getReceiver().getServerId();
            model.receiverType = Message.RECEIVER_TYPE_NONE;
        } else {
            model.receiverId = message.getGroup().getServerId();
