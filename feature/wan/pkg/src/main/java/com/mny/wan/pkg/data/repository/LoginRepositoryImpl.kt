...
class LoginRepositoryImpl @Inject constructor() : BaseRepository(), LoginRepository {
    ...
    override suspend fun login(username: String, password: String): MojitoResult<UserInfoDomainModel> {
        return executeRequest<UserInfoDomainModel>(dataBlock = {
            ...
            emit(MojitoResult.Success(response.data.toDomainModel()))
        }, errorBlock = {
            emit(MojitoResult.Error(Exception("有错误")))
        })
    }
    ...
    override suspend fun register(
        ...
        password: String,
        rePassword: String
    ): MojitoResult<UserInfoDomainModel> {
