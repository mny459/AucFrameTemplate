...
class CoinRankViewModel @ViewModelInject constructor(
    private val mUseCase: ArticleUseCase,
    @Assisted private  val mSavedStateHandle: SavedStateHandle
) :
    BaseViewModel<CoinRankViewModel.ViewState, CoinRankViewModel.Action>(ViewState()) {
    ...
    val mCoinRank: Flow<PagingData<BeanRanking>> = flowOf(
        mClearListCh.receiveAsFlow().map { PagingData.empty() },
